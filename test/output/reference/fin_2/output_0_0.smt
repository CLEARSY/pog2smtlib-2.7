...
(assert (!
  ...
  :named |Define:lprp:1|))
(assert (!
  (|set.subseteq POW Z| p2 (|finite sub-sets Z| INTEGER))
  :named |Define:lprp:2|))
(assert (!
  (|set.subseteq POW Z| p3 (|finite sub-sets Z| INTEGER))
  :named |Define:lprp:3|))
(assert (!
  (|set.subseteq POW Z| p1 p2)
  :named |Define:lprp:4|))
(assert (!
  (|set.subseteq POW Z| p2 p3)
  :named |Define:lprp:5|))
(assert (!
  (not (|set.subseteq POW POW Z| (|sub-sets POW Z| p1) (|sub-sets POW Z| p3)))
  :named |Goal|))
(check-sat)
(exit)
