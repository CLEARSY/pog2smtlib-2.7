...
(assert (!
  (not (|set.in POW (Z x Z)| (|set.intent (Z x Z)| (lambda ((x |(Z x Z)|)) (or (= x (maplet 0 1))))) (|bijections.total Z Z| (|set.intent Z| (lambda ((x |Z|)) (or (= x 0)))) (|set.intent Z| (lambda ((x |Z|)) (or (= x 0)(= x 1)))))))
  :named |Goal|)
)
(check-sat)
(exit)
