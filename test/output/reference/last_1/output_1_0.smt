...
(assert (!
  (not
    (|set.in POW (Z x Z)| (|set.intent (Z x Z)| (lambda ((x |(Z x Z)|)) (or (= x (maplet 1 5))(= x (maplet 2 5))(= x (maplet 3 7))))) (|seq Z| (|rel.range Z Z| (|set.intent (Z x Z)| (lambda ((x |(Z x Z)|)) (or (= x (maplet 1 5))(= x (maplet 2 5))(= x (maplet 3 7)))))))))
  :named |Goal|))
(check-sat)
(exit)
