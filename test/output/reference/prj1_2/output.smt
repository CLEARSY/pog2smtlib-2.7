...
(assert (!
  (not (= (|prj1 Z Z| (|set.intent Z| (lambda ((x |Z|)) (or (= x 0)(= x 1)))) (|set.intent Z| (lambda ((x |Z|)) (or (= x 3)(= x 2))))) (|set.intent ((Z x Z) x Z)| (lambda ((x |((Z x Z) x Z)|)) (or (= x (maplet (maplet 0 3) 0))(= x (maplet (maplet 0 2) 0))(= x (maplet (maplet 1 3) 1))(= x (maplet (maplet 1 2) 1)))))))
  :named |Goal|)
)
(check-sat)
(exit)
