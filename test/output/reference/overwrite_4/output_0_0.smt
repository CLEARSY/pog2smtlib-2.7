...
(assert (!
  (not
    (= (|rel.overwrite Z Z| |set.empty (Z x Z)| (|set.intent (Z x Z)| (lambda ((x |(Z x Z)|)) (or (= x (maplet 0 1))(= x (maplet 1 1))(= x (maplet 1 2)))))) (|set.intent (Z x Z)| (lambda ((x |(Z x Z)|)) (or (= x (maplet 0 1))(= x (maplet 1 1))(= x (maplet 1 2)))))))
  :named |Goal|))
(check-sat)
(exit)
