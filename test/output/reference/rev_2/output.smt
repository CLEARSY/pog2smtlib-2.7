...
(assert (!
  (not (= (|rev Z| (|set.intent (Z x Z)| (lambda ((x |(Z x Z)|)) (or (= x (maplet 0 1))(= x (maplet 1 5))(= x (maplet 2 2))(= x (maplet 3 9)))))) (|set.intent (Z x Z)| (lambda ((x |(Z x Z)|)) (or (= x (maplet 0 9))(= x (maplet 1 2))(= x (maplet 2 5))(= x (maplet 3 1)))))))
  :named |Goal|)
)
(check-sat)
(exit)
