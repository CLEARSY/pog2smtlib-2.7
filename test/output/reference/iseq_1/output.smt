...
(assert (!
  (forall ((X |POW Z|) (Y |POW POW Z|) (f |POW (Z x POW Z)|))
     (= (|set.in POW (Z x POW Z)| f (|injections Z POW Z| X Y))
        (forall ((p1 |(Z x POW Z)|) (p2 |(Z x POW Z)|))
          (=> (and (|set.in (Z x POW Z)| p1 f) (|set.in (Z x POW Z)| p2 f) (= (snd p1) (snd p2)))
              (= (fst p1) (fst p2))))))
...
(assert (!
  ...
  :named |ax.sub-sets Z|))
(assert (!
  (not (|set.in POW (Z x POW Z)| (|set.intent (Z x POW Z)| (lambda ((x |(Z x POW Z)|)) (or (= x (maplet 0 (|set.intent Z| (lambda ((x |Z|)) (or (= x 0)(= x 1)(= x 0))))))(= x (maplet 1 (|set.intent Z| (lambda ((x |Z|)) (or (= x 0))))))(= x (maplet 2 (|set.intent Z| (lambda ((x |Z|)) (or (= x 0)(= x 1)(= x 0))))))))) (|iseq POW Z| (|sub-sets Z| INTEGER))))
  :named |Goal|)
)
(check-sat)
(exit)
