...
(assert (!
  (not (= (|parallelproduct Z Z Z Z| (|set.intent (Z x Z)| (lambda ((x |(Z x Z)|)) (= x (maplet 0 1)))) (|set.intent (Z x Z)| (lambda ((x |(Z x Z)|)) (= x (maplet 7 10))))) (|set.intent ((Z x Z) x (Z x Z))| (lambda ((x |((Z x Z) x (Z x Z))|)) (= x (maplet (maplet 0 7) (maplet 1 10)))))))
  :named |Goal|))
(check-sat)
(exit)
