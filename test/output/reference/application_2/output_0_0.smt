...
(assert (!
  (not
    (= (|fun.eval Z POW Z| (|set.intent (Z x POW Z)| (lambda ((x |(Z x POW Z)|)) (or (= x (maplet 0 (|set.intent Z| (lambda ((x |Z|)) (or (= x 1)(= x 3)(= x 4))))))(= x (maplet 1 (|set.intent Z| (lambda ((x |Z|)) (or (= x 1)(= x 3)(= x 5))))))(= x (maplet 3 (|set.intent Z| (lambda ((x |Z|)) (or (= x 1)(= x 6)(= x 4))))))(= x (maplet 4 (|set.intent Z| (lambda ((x |Z|)) (or (= x 1)(= x 1)(= x 4))))))))) 3) (|set.intent Z| (lambda ((x |Z|)) (or (= x 1)(= x 6)(= x 4))))))
  :named |Goal|))
(check-sat)
(exit)
