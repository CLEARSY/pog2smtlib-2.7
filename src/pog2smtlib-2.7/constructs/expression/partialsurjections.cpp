...
(assert (!
  ...
  :named |ax:def.psurj {8}|))
)";

namespace Expression {

MapBinaryBType<Partial_Surjection> Partial_Surjection::m_cache;

Partial_Surjection::Partial_Surjection(const BType &U, const BType &V,
                                       const string &script,
...
shared_ptr<Abstract> Factory::Partial_Surjection(const BType &U,
  ...
  shared_ptr<Abstract> result =
      find(BConstruct::Expression::Partial_Surjection::m_cache, U, V);
  if (!result) {
    ...
    const std::string script = fmt::format(
        SCRIPT, /*0*/ smtSymbol(Expr::BinaryOp::Partial_Surjections, U, V),
        /*1*/ symbol(PU),
        /*2*/ symbol(PV),
        /*3*/ symbol(PPUxV),
        /*4*/ symbol(PUxV),
    ...
    result = make(BConstruct::Expression::Partial_Surjection::m_cache, U, V,
                  script, requisites);
  }
  return result;
}
