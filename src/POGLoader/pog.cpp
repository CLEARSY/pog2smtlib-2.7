...
BType readType(const QDomElement &dom) {
  if (dom.isNull()) throw pog::PogException("Null dom element.");
  ...
  if (tag == "Id") {
    ...
    if (value == "INTEGER") {
      ...
      if (dom.hasAttribute("suffix"))
        throw pog::PogException(
            "Abstract or Concrete Set with suffix.");  // this constraint
                                                       // could be
                                                       // removed
      return BType::INT;
    }
    ...
    for (QDomElement item = dom.firstChildElement("Record_Item");
         !item.isNull(); item = item.nextSiblingElement("Record_Item")) {
      fields.push_back({item.attribute("label").toStdString(),
...
void readTypeInfos(const QDomElement &dom, std::vector<BType> &typeInfos) {
  ...
  if (!dom.isNull()) {
    int cpt = 0;
    for (QDomElement typ = dom.firstChildElement("Type"); !typ.isNull();
         typ = typ.nextSiblingElement()) {
      bool ok = false;
      int typref = typ.attribute("id").toInt(&ok);
      if (!ok) throw pog::PogException("Integer expected");
      if (typref != cpt)
...
pog::Set readSet(const std::vector<BType> &typeInfos, const QDomElement &dom) {
  ...
  QDomElement elt =
      dom.firstChildElement("Enumerated_Values").firstChildElement("Id");
  while (!elt.isNull()) {
    vec.push_back(Xml::VarNameFromId(elt, typeInfos));
    elt = elt.nextSiblingElement("Id");
  }
  QDomElement id = dom.firstChildElement("Id");
  if (id.isNull()) throw pog::PogException("Missing 'Id' element.");
...
pog::Pog pog::read(const QDomDocument &pog) {
  ...
  if (root.isNull()) throw PogException("Proof_Obligations element expected.");
  // TypeInfos
  std::vector<BType> typeInfos;
  readTypeInfos(root.firstChildElement("TypeInfos"), typeInfos);
  // Defines
  for (QDomElement e = root.firstChildElement("Define"); !e.isNull();
       e = e.nextSiblingElement("Define")) {
    if (!e.hasAttribute("name")) throw PogException("Attribute name expected.");
    ...
    for (QDomElement ch = e.firstChildElement(); !ch.isNull();
  ...
  for (QDomElement po = root.firstChildElement("Proof_Obligation");
       !po.isNull(); po = po.nextSiblingElement("Proof_Obligation")) {
    // goalHash
    size_t goalHash = 0;
    ...
    for (QDomElement e = po.firstChildElement("Definition"); !e.isNull();
         e = e.nextSiblingElement("Definition")) {
      definitions.push_back(e.attribute("name").toStdString());
    }
    // Hypothesis
    std::vector<Pred> hyps;
    for (QDomElement e = po.firstChildElement("Hypothesis"); !e.isNull();
    ...
    for (QDomElement e = po.firstChildElement("Local_Hyp"); !e.isNull();
    ...
    for (QDomElement e = po.firstChildElement("Simple_Goal"); !e.isNull();
      ...
      for (QDomElement ch = e.firstChildElement("Ref_Hyp"); !ch.isNull();
    ...
    res.pos.push_back(POGroup(tag, goalHash, definitions, std::move(hyps),
