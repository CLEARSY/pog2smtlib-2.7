...
class SmtFunDecl {
 public:
  ...
  std::string addComprehensionSet(const Expr::QuantifiedSet &p, const string s);
...
std::string SmtFunDecl::addFunction(int nbParams) {
  ...
  std::string args = "";
  for (int i = 0; i < nbParams; i++) args += " " + type;
...
std::string predToSmtLib(const Pred &f, SmtFunDecl &decls,
  ...
  switch (f.getTag()) {
        ...
        if (p.rhs.getTag() == Expr::EKind::BinaryExpr) {
          ...
          if (rhs.op == Expr::NaryOp::Set) {
            ...
            if (rhs.vec.size() == 0)
              ...
              return "(= " + a + " " +
                     exprToSmtLib(rhs.vec.at(0), decls, bv, used_ids) + ")";
            else {
              std::string accu = "(or";
              for (auto &e : rhs.vec)
                accu += " (= " + a + " " +
    ...
    case Pred::PKind::Conjunction: {
      ...
      if (p.operands.size() == 0) return "true";
    ...
    case Pred::PKind::Disjunction: {
      ...
      if (p.operands.size() == 0) return "false";
      if (p.operands.size() == 1)
        return predToSmtLib(p.operands.at(0), decls, bv, used_ids);
      std::string accu;
      for (auto &e : p.operands) {
...
std::string exprToSmtLib(const Expr &f, SmtFunDecl &decls,
  ...
  switch (f.getTag()) {
    ...
    case Expr::EKind::Id: {
      VarName v = f.getId();
      if (bv.find(v) != bv.end()) return "l_" + varNameToString(v);
    ...
    case Expr::EKind::Record_Field_Update: {
      ...
      for (auto &v : f.getFreeVars()) {
        if (bv.find(v) != bv.end()) fv.insert(v);
      }
      if (fv.size() == 0) {
        std::string s = decls.addConstant(f);
        used_ids.insert(s);
      ...
      std::string vars;
      for (auto &v : fv) vars += " l_" + varNameToString(v);
...
void merge(std::set<std::string> &set, const std::set<std::string> &to_insert) {
  for (auto &e : to_insert) set.insert(e);
}
...
void writeNonIncrOutput(SmtFunDecl &decls,
  ...
  for (auto &ref : sg.localHypsRef) {
    ...
    if (it != localHyps_tr.end()) {
      ...
      std::string s = predToSmtLib(localHyps[ref - 1], decls, {}, used_ids2);
  ...
  if (produce_model) out << "(set-option :produce-models true)\n";
  out << "(set-logic ALL)\n";
  ...
  out << "; Defines\n";
  for (auto &d : defines) out << d << "\n";
  out << "; Global hypotheses\n";
  for (auto &hyp : globalHyps) out << "(assert " << hyp << ")\n";
  ...
  out << "(assert (not " << goal << "))\n";
  out << "(check-sat)\n";
  if (produce_model) out << "(get-model)\n";
  out << "(exit)";
  out.close();
}
...
void saveSmtLibFileNonIncrGroupSome(
  ...
  for (auto i : goals) {
    ...
    writeNonIncrOutput(decls, defines, globalHyps, used_ids, group.localHyps,
                       localHyps_tr, group.tag, groupId, i,
...
std::string simpleGoalToSmtLib(const pog::PO &sg, SmtFunDecl &decls) {
  ...
  std::string accu = "(and";
  for (const auto &ref : sg.localHypsRef) accu += " lh_" + std::to_string(ref);
...
void writeSmtLibFileIncr(Logic, const pog::Pog &pog,
  ...
  for (const auto &def : pog.defines) {
    ...
    switch (nbChildren) {
      case 0:
        defines.push_back("(define-fun |def_" + def.name + "| () Bool true)");
        break;
      case 1:
        defines.push_back("(define-fun |def_" + def.name + "| () Bool " + fm +
                          ")");
        break;
      default:
        defines.push_back("(define-fun |def_" + def.name + "| () Bool (and" +
                          fm + "))");
    }
  }
  ...
  out << decls.getAxioms();
  out << "; Opaque formulas\n";
  for (auto &d : decls.getSmtLibDeclarations()) out << d.second << "\n";
  out << "; Defines\n";
  for (auto &d : defines) out << d << "\n";
  for (Po po : pos) {
    out << "; PO group " << po.group << " \n";
    if (pos.size() > 1) out << "(push 1)\n";
    for (auto &def : po.definitions) out << "(assert |def_" << def << "|)\n";
    for (auto &hyp : po.hypotheses) out << "(assert " << hyp << ")\n";
...
void saveSmtLibFileIncrSome(Logic logic, const pog::Pog &pog,
  ...
  for (size_t group = 0; group < pog.pos.size(); ++group) {
    ...
    if (it == goals.end()) continue;
    auto &simple_goal_ids{it->second};
    if (simple_goal_ids.size() == 0) continue;
    ...
    for (size_t i = 0; i < po.simpleGoals.size(); i++) {
      if (simple_goal_it == simple_goal_ids.end()) break;
      if (i == *simple_goal_it) {
...
void saveSmtLibFileNonIncr(Logic logic, const pog::Pog &pog,
  ...
  for (size_t i = 0; i < pog.pos.size(); i++) {
    ...
    saveSmtLibFileNonIncrGroupAll(decls, pog.defines, all_defines_tr, groupId,
                                  pog.pos[groupId], output, produce_model);
  }
}
}  // namespace smtlib
